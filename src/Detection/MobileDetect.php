...
namespace Shaqi\MobileDetect\Detection;
 ...
 * It uses the User-MobileDetect string combined with specific HTTP headers to detect the mobile environment.
 *
...
class MobileDetect
{
    ...
    /**
     * The User-MobileDetect HTTP header is stored in here.
     * @var string
     */
    protected $userMobileDetect = null;
    ...
    protected static $operatingSystems = array(
        ...
        // http://nicksnettravels.builttoroam.com/post/2011/01/10/Bogus-Windows-Phone-7-User-MobileDetect-String.aspx
    ...
    /**
     * List of mobile User MobileDetects.
     *
     ...
     * User-MobileDetect string.
     *
     * @var array
     */
    protected static $uaHttpHeaders = array(
        // The default User-MobileDetect string.
        'HTTP_USER_AGENT',
    ...
    /**
     * The individual segments that could exist in a User-MobileDetect string. VER refers to the regular
    ...
    protected static $properties = array(
        ...
        'Dolfin'        => 'Dolfin/[VER]',
        // @reference: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/User-MobileDetect/Firefox
     ...
     * @param string $userMobileDetect Inject the User-MobileDetect header. If null, will use HTTP_USER_AGENT
     *                          from the $headers array instead.
     */
    public function __construct(
        array $headers = null,
        $userMobileDetect = null
    ) {
        $this->setHttpHeaders($headers);
        $this->setUserMobileDetect($userMobileDetect);
    }
     ...
     *                       "User-MobileDetect" or "X-Device-User-MobileDetect" or can be php-esque with the
     ...
     * can contain the User-MobileDetect string.
     *
    ...
    /**
     * @param string $userMobileDetect
     * @return string
     */
    private function prepareUserMobileDetect($userMobileDetect) {
        $userMobileDetect = trim($userMobileDetect);
        $userMobileDetect = substr($userMobileDetect, 0, 500);
        return $userMobileDetect;
    }

    /**
     * Set the User-MobileDetect to be used.
     *
     * @param string $userMobileDetect The user agent string to set.
     *
     * @return string|null
     */
    public function setUserMobileDetect($userMobileDetect = null)
    {
        // Invalidate cache due to #375
        $this->cache = array();

        if (false === empty($userMobileDetect)) {
            return $this->userMobileDetect = $this->prepareUserMobileDetect($userMobileDetect);
        } else {
            $this->userMobileDetect = null;
            foreach ($this->getUaHttpHeaders() as $altHeader) {
                if (false === empty($this->httpHeaders[$altHeader])) { // @todo: should use getHttpHeader(), but it would be slow. (Serban)
                    $this->userMobileDetect .= $this->httpHeaders[$altHeader] . " ";
                }
            }

            if (!empty($this->userMobileDetect)) {
                return $this->userMobileDetect = $this->prepareUserMobileDetect($this->userMobileDetect);
            }
        }

        if (count($this->getCfHeaders()) > 0) {
            return $this->userMobileDetect = 'Amazon CloudFront';
        }
        return $this->userMobileDetect = null;
    }

    /**
     * Retrieve the User-MobileDetect.
     *
     * @return string|null The user agent if it's set.
     */
    public function getUserMobileDetect()
    {
        return $this->userMobileDetect;
    }
     ...
     */
    public static function getUserMobileDetects()
    {
     ...
     *
     * @param  null    $userMobileDetect deprecated
     * @return boolean
     */
    protected function matchDetectionRulesAgainstUA($userMobileDetect = null)
    {
        ...
        foreach ($this->getRules() as $_regex) {
            ...
            if ($this->match($_regex, $userMobileDetect)) {
                return true;
            }
        }
     ...
     * regex against the User-MobileDetect.
     *
     * @param string $key
     *
     * @return boolean
     ...
     * @return bool
     */
    public function isMobile($userMobileDetect = null, $httpHeaders = null)
    {
        ...
        if ($this->getUserMobileDetect() === 'Amazon CloudFront') {
     ...
     *
     * @param  string $userMobileDetect   deprecated
     * @param  array  $httpHeaders deprecated
     * @return bool
     */
    public function isTablet($userMobileDetect = null, $httpHeaders = null)
    {
        // Check specifically for cloudfront headers if the useragent === 'Amazon CloudFront'
        if ($this->getUserMobileDetect() === 'Amazon CloudFront') {
            $cfHeaders = $this->getCfHeaders();
     ...
     */
    public function is($key, $userMobileDetect = null, $httpHeaders = null)
    {
     ...
     * their conventions in representing the User-MobileDetect or
     * the HTTP headers.
     *
     * This method will be used to check custom regexes against
     * the User-MobileDetect string.
     *
     * @param $regex
     * @param string $userMobileDetect
     * @return bool
     ...
     */
    public function match($regex, $userMobileDetect = null)
    {
        if (!\is_string($userMobileDetect) && !\is_string($this->userMobileDetect)) {
            return false;
        }

        $match = (bool) preg_match(sprintf('#%s#is', $regex), (false === empty($userMobileDetect) ? $userMobileDetect : (is_string($this->userMobileDetect) ? $this->userMobileDetect : '')), $matches);
    ...
    /**
     * Check the version of the given property in the User-MobileDetect.
    ...
    public function version($propertyName, $type = self::VERSION_TYPE_STRING)
    {
        ...
        if (!\is_string($this->userMobileDetect)) {
            return false;
        }
        ...
        if (true === isset($properties[$propertyName])) {
            ...
            foreach ($properties[$propertyName] as $propertyMatchString) {
                ...
                preg_match(sprintf('#%s#is', $propertyPattern), (is_string($this->userMobileDetect) ? $this->userMobileDetect : ''), $match);
